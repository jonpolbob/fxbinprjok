...
#attention , selon les ecrans il peut arriver que le lien ne soit jamais visible , d'oiu le scroll des que le clic échour
# pour  faire remonter la ligne a cliquer dans le visible
...
def initchromdriver():
    ...
    prefs = {"download.default_directory": "c:/tmp/fxzip"}
...
def loadpairemoisan(paire,mois,an):
    ...
    print("wait to clic on file ",FileName)

    window = driver.find_element_by_class_name('page') #il ya atoujours une classe avec ce nom
    ...
    print("clic done")
    pathdownload = "c:/tmp/fxzip/"+RealFileName #attention le nom est different entre le zip et le lien clic
    ...
    while notok:
        try:
            with zipfile.ZipFile(pathdownload, 'r') as zf:
                ...
                for i in zf.filelist:
                    if i.filename.find(".csv") != -1 :
                        print("extract dans c:\\tmp\\fxzip : ",i.filename)
                        newpath = zf.extract(i,path="c:/tmp")
                        print('new path ',newpath, 'rename to',"c:\\tmp\\fxzip\\"+paire+".csv")
                        os.rename(newpath,"c:\\tmp\\fxzip\\"+paire+".csv")
                        notok = False
            ...
            #zf.close()
            if os._exists("c:\\tmp\\fxzip\\" + paire + ".csv"):
                os.remove("c:\\tmp\\fxzip\\" + paire + ".csv")
            print('redo')


    os.remove(pathdownload)

    return newpath,"c:\\tmp\\fxzip\\"+paire+".csv"

#fonction lisant un mois pour la liste  des paires passees en argument
#range tous les fichiers de la liste des paires dans un seul zip
def liremois(mois,annee,listepairs):

    listefic = []  # liste des fichier extraits
    #on efface tous les csv du repetroire
    for todel in os.listdir("c:\\tmp\\fxzip"):
        if '.csv' in todel:
            os.remove("c:\\tmp\\fxzip\\"+todel)
